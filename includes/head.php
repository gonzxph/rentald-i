<!-- Set the character encoding for the document -->
<meta charset="UTF-8">

<!-- Set the viewport to ensure proper scaling on different devices -->
<meta name="viewport" content="width=device-width, initial-scale=1.0">

<!-- Link to the main stylesheet for custom styles -->
<link rel="stylesheet" href="style/css/style.css">

<!-- Link to Bootstrap CSS for responsive design and pre-built components -->
<link href="style/css/bootstrap.min.css" rel="stylesheet" crossorigin="anonymous">

<!-- Link to Font Awesome for icon fonts -->
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">

<!-- Link to Vanilla Calendar Pro CSS for calendar styling -->
<link href="style/css/vanilla-calendar.min.css" rel="stylesheet">

<!-- Link to Material Design Icons for additional icon fonts -->
<link href="https://cdn.jsdelivr.net/npm/@mdi/font@7.4.47/css/materialdesignicons.min.css" rel="stylesheet">

<!-- SweetAlert2 CSS -->
<link rel="stylesheet" href="style/css/sweetalert2.min.css">